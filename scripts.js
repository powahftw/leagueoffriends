    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.response['success'] == false) {
                setarray("error in id", arr, idx);
                arr[idx].toupdate = false;
            } else if (data.response['success'] == true && data.response['in_game'] == false) {
                if (arr[idx].status != "not in game") {
                    notifyMe(arr, idx);
                    arr[idx].toupdate = false;
                    setarray("end game", arr, idx);
                } else {
                    setarray("not in game", arr, idx);
                }
...
function updatevalue(arr) {
    if (arr != undefined && arr.length > 0) {
        for (var i in arr) {
            if (arr[i].toupdate) {
                var callurl = "https://apirestflask.herokuapp.com/" + arr[i].server.toLowerCase() + "/" + arr[i].name;
                console.log(callurl)
...
function genUI(arr) {
    ...
    for (var i in arr) {
        if (arr[i].status == "end game") {
            var targetclass = "endgame"
        } else if (arr[i].status == "not in game") {
            var targetclass = "notingame"
        } else if (arr[i].status == "error in id") {
            var targetclass = "errorinid"
        } else {
            var targetclass = "ingame"
        }
        
        var server_nick = '<div class="infobox col-6">' + String(arr[i].server).toUpperCase().replace(1, "") + " " + String(arr[i].name).toUpperCase() + '</div>'
        var closingx = "<a href='#' class='close' aria-hidden='true'>&times;</a>"
        var statustag = '<div class="col-3 ' + targetclass.split(" ")[0] + '">' + String(arr[i].status.toUpperCase()) + "</div>"
        $("<ul>" + server_nick + " " + statustag + " " + closingx + "</ul>").addClass("custombox").appendTo($('#todo'));
    }
}
...
$(document).ready(function() {
    ...
    // forgive me father because i have sinned, no p{L} in js 

    var firstNotification = 0;
    var rx = RegExp("^[0-9\\" + pL + "_\\.]+$");
    
    setInterval(function() {
        updatevalue(arr)
    }, 8000 + 4000 * arr.length);

    function addElement() {
        var unique = 1;
        for (var i in arr) { // Loop to check if elements is already in the array
            
            if ($('#dropbown').find(":selected").val() == arr[i].server &&
                $("input[name=task]").val().replace(/\s/g, "") == arr[i].name) {
                unique = 0;
            }
        }
        var inputname = $("input[name=task]").val().replace(/\s/g, "");
        if (2 < inputname.length &&     // Not too short nicknames
            inputname.length < 17 &&    // Too long nicknames
            unique &&                   // Not already there
            rx.test(inputname) &&       // Regex provided by Rito
            arr.length < 4              // Not too many people
            ...
            var newEntry = {
                server: $('#dropbown').find(":selected").val(),
                name: inputname.replace(" ", "").toLowerCase().replace(/\s/g, ""),
                status: "not in game",
                toupdate: true,
            }
            arr.push(newEntry);
    ...
    $('button').click(function() { // Hanlde click on Add! button
        addElement();
    });
    
    $(window).keydown(function(event){ // Handle ENTER on KeyBoard
        if(event.keyCode == 13) {
    ...
    $("body").on('click', '#todo a', function() {
        ...
        for (var i in arr) { // Find index of the element to remove
            ...
            if (string[0] == arr[i].server.toUpperCase().replace(1, "") &&
                string[1] === arr[i].name.toUpperCase())
                todelete = i
        }
...
function notifyMe(arr, idx) {
  ...
  notificationSound.setAttribute('src', 'https://freesound.org/data/previews/234/234524_4019029-lq.mp3');
